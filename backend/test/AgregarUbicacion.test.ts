...
test('rechaza una dirección trivial y conserva el mensaje de Domain', async () => {
  const { handler: sut } = handler(informacion());
  await assert.rejects(
    sut.execute(new AgregarUbicacionCommand('a', 10, -66)),
    (error: UbicacionRechazadaException) => {
      assert.deepEqual(error.errors, [{
        field: 'direccion',
        message: 'La dirección es demasiado corta. Escriba una dirección más completa.',
      }]);
      assert.equal(error.message, 'La dirección es demasiado corta. Escriba una dirección más completa.');
      return true;
    },
  );
});
...
test('rechaza coordenadas fuera de rango o no finitas', async () => {
  ...
  for (const [lat, lon] of invalidas) {
    const { handler: sut } = handler(informacion());
    await assert.rejects(sut.execute(new AgregarUbicacionCommand('Dirección válida', lat, lon)), UbicacionRechazadaException);
  }
});

test('rechaza cuando ya existe una ubicación (HU24 no sobrescribe)', async () => {
  const info = informacion();
  info.setLocation(new CompanyLocation(new Address('Ubicación existente'), new GeoCoordinates(1, 2)));
  const { handler: sut } = handler(info);
  await assert.rejects(
    sut.execute(new AgregarUbicacionCommand('Ubicación nueva', 3, 4)),
    /Ya existe una ubicación registrada/,
  );
  // La ubicación previa no se sobrescribe.
  assert.equal(info.location?.address.value, 'Ubicación existente');
});
...
test('rechaza cuando el CompanyProfile no ha sido inicializado', async () => {
  ...
  await assert.rejects(sut.execute(new AgregarUbicacionCommand('Dirección válida', 10, -66)), /no ha sido inicializada/);
});
