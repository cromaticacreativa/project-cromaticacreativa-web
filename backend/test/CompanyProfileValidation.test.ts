...
test('Address normaliza y acepta direcciones estructuralmente completas', () => {
  assert.equal(new Address('  Av. Principal, Caracas  ').value, 'Av. Principal, Caracas');
  for (const valida of [
    'Av. Principal, Edificio X, Caracas, Venezuela',
    'Calle 10, Residencias Las Palmeras, Caracas',
    'Av. Francisco de Miranda, Chacao, Caracas',
    'Centro Comercial X, Nivel 2, Local 15, Caracas',
  ]) {
    assert.equal(new Address(valida).value, valida);
  }
});

test('Address rechaza faltantes, vacíos y valores triviales con mensajes claros', () => {
...
test('Address aplica exactamente el mínimo 10 y el máximo 500 después de trim', () => {
  const minima = 'a'.repeat(10);
  const maxima = 'a'.repeat(500);
  assert.equal(new Address(`  ${minima}  `).value, minima);
  assert.throws(
    () => new Address('a'.repeat(9)),
    /La dirección es demasiado corta\. Escriba una dirección más completa\./,
  );
  assert.equal(new Address(maxima).value, maxima);
  assert.throws(
    () => new Address('a'.repeat(501)),
