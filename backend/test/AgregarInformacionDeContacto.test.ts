...
import { AgregarCorreoReceptorStrategy, TIPO_CORREO_RECEPTOR } from '../src/modules/CompanyProfile/CompanyProfile.Application/Strategies/AgregarCorreoReceptorStrategy';
...
import { IResultadoInformacionDeContactoOrdenado } from '../src/modules/CompanyProfile/CompanyProfile.Application/Ports/IResultadoInformacionDeContacto';
...
function estrategiasReales(): IAgregarInformacionDeContactoStrategy[] {
  const validadoraCorreo = new ValidadoraCorreo();
  return [
    new AgregarTelefonoStrategy(new ValidadoraTelefono()),
    new AgregarCorreoStrategy(validadoraCorreo),
    new AgregarCorreoReceptorStrategy(validadoraCorreo),
    new AgregarRedSocialStrategy(new ValidadoraRedSocial()),
  ];
}
...
test('el Handler orquesta cada medio a través de su Strategy (teléfono, correo, red)', async () => {
  ...
  assert.ok('displayOrder' in telefono);
...
test('el Handler carga una vez el Aggregate y resuelve el correo receptor por Strategy', async () => {
  const info = informacion();
  const { handler: sut, reader } = handler(info);
  const resultado = await sut.execute(comando({
    tipo: TIPO_CORREO_RECEPTOR,
    datos: { correo: 'Solicitudes@EMPRESA.COM' },
  }));
  assert.equal(reader.llamadas, 1);
  assert.deepEqual(resultado, {
    tipo: TIPO_CORREO_RECEPTOR,
  ...
  assert.equal(info.contactRequestRecipientEmail.value, 'Solicitudes@empresa.com');
});
...
test('el resolver rechaza el correo receptor cuando ninguna Strategy soporta la entrada', async () => {
  const soloCorreoPublico: IAgregarInformacionDeContactoStrategy[] = [
    new AgregarCorreoStrategy(new ValidadoraCorreo()),
  ];
  const { handler: sut } = handler(informacion(), soloCorreoPublico);
  await assert.rejects(
    sut.execute(comando({ tipo: TIPO_CORREO_RECEPTOR, datos: { correo: 'a@b.com' } })),
    /No hay una estrategia que soporte/,
  );
});

test('el resolver trata como error de configuración múltiples Strategy de correo receptor', async () => {
  const validadora = new ValidadoraCorreo();
  const dosCorreosReceptores: IAgregarInformacionDeContactoStrategy[] = [
    new AgregarCorreoReceptorStrategy(validadora),
    new AgregarCorreoReceptorStrategy(validadora),
  ];
  const { handler: sut } = handler(informacion(), dosCorreosReceptores);
  await assert.rejects(
    sut.execute(comando({ tipo: TIPO_CORREO_RECEPTOR, datos: { correo: 'a@b.com' } })),
...
test('OCP: un medio totalmente nuevo se soporta sin modificar el Handler ni un catálogo central', async () => {
  ...
  const nuevaEstrategia: IAgregarInformacionDeContactoStrategy = {
    ...
    ejecutar: (info, entrada): IResultadoInformacionDeContactoOrdenado => {
      marcador.usado = true;
      return {
        tipo: entrada.tipo,
