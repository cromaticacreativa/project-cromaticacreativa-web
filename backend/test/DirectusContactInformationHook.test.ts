...
type FilterHandler = (payload: Record<string, unknown>, meta?: Record<string, unknown>) => Promise<Record<string, unknown>>;
...
test('el Hook intercepta create y update de children, ubicación y el correo receptor', async () => {
  const filtros = await registrarFiltros();
  assert.deepEqual(
    [...filtros.keys()].sort(),
    [
      'company_profile.items.update',
      'email.items.create', 'email.items.update',
      'location.items.create', 'location.items.update',
      'phone.items.create', 'phone.items.update',
      'social_link.items.create', 'social_link.items.update',
    ],
  );
  // La eliminación NO pasa por el Hook (Directus directo).
  assert.equal(filtros.has('phone.items.delete'), false);
  assert.equal(filtros.has('location.items.delete'), false);
});

test('un update de teléfono aprobado usa /modify con id y descarta campos maliciosos (allowlist)', async () => {
  ...
  globalThis.fetch = (async (url: string, init: RequestInit) => {
    ...
    return { ok: true, status: 200, json: async () => ({ payload: { number: '+584241112233' } }) };
  }) as unknown as typeof fetch;
  try {
    const filtros = await registrarFiltros();
    // El navegador intenta colar company_profile_id/display_order/id: NO sobreviven.
    const salida = await filtros.get('phone.items.update')!(
      { number: ' +58 0424 1112233 ', company_profile_id: 'otro', display_order: 999, id: 'suplantado' },
      { keys: ['id-1'] },
    );
    assert.deepEqual(salida, { number: '+584241112233' });
    assert.equal(solicitud!.url, 'http://localhost:3000/internal/cms/company-profile/contact-information/modify');
    assert.deepEqual(solicitud!.body, {
      collection: 'phone',
      id: 'id-1',
      payload: { number: ' +58 0424 1112233 ', company_profile_id: 'otro', display_order: 999, id: 'suplantado' },
    });
  } finally {
...
test('un update de ubicación aprobado devuelve solo los campos canónicos (allowlist)', async () => {
  const restaurar = conEntorno({ BACKEND_INTERNAL_URL: 'http://localhost:3000', BACKEND_INTERNAL_TOKEN: 'secreto' });
  globalThis.fetch = (async () => ({ ok: true, status: 200, json: async () => ({ payload: { address: 'Av. X', latitude: 10.5, longitude: -66.9 } }) })) as unknown as typeof fetch;
  try {
    const filtros = await registrarFiltros();
    const salida = await filtros.get('location.items.update')!(
      { address: 'Av. X', latitude: 10.5, longitude: -66.9, company_profile_id: 'otro' },
      { keys: ['perfil-1'] },
    );
    assert.deepEqual(salida, { address: 'Av. X', latitude: 10.5, longitude: -66.9 });
  } finally {
    globalThis.fetch = originalFetch;
    restaurar();
  }
...
test('un update de red social rechazado (422) se relanza como DirectusError y NO persiste', async () => {
  const restaurar = conEntorno({ BACKEND_INTERNAL_URL: 'http://localhost:3000', BACKEND_INTERNAL_TOKEN: 'secreto' });
  globalThis.fetch = (async () => ({ ok: false, status: 422, json: async () => ({ statusCode: 422, message: 'La URL debe ser una URL HTTP o HTTPS válida.', errors: [{ field: 'url', message: 'La URL debe ser una URL HTTP o HTTPS válida.' }] }) })) as unknown as typeof fetch;
  try {
    const filtros = await registrarFiltros();
    await assert.rejects(filtros.get('social_link.items.update')!({ url: 'hola' }, { keys: ['id-9'] }), (error: unknown) => {
...
test('un update de ubicación con backend caído cancela (genérico, fail closed)', async () => {
  const restaurar = conEntorno({ BACKEND_INTERNAL_URL: 'http://localhost:3000', BACKEND_INTERNAL_TOKEN: 'secreto' });
  globalThis.fetch = (async () => { throw new Error('ECONNREFUSED'); }) as unknown as typeof fetch;
  try {
    const filtros = await registrarFiltros();
    await assert.rejects(filtros.get('location.items.update')!({ address: 'Nueva dirección' }, { keys: ['perfil-1'] }), /No fue posible procesar/);
  } finally {
    globalThis.fetch = originalFetch;
    restaurar();
  }
...
test('un update de child sin id único se cancela (fail closed, no contacta al backend)', async () => {
  const restaurar = conEntorno({ BACKEND_INTERNAL_URL: 'http://localhost:3000', BACKEND_INTERNAL_TOKEN: 'secreto' });
  let llamado = false;
  globalThis.fetch = (async () => { llamado = true; return { ok: true, status: 200, json: async () => ({ payload: {} }) }; }) as unknown as typeof fetch;
  try {
    const filtros = await registrarFiltros();
    await assert.rejects(filtros.get('email.items.update')!({ address: 'a@b.com' }, { keys: ['id-1', 'id-2'] }), /No fue posible procesar/);
