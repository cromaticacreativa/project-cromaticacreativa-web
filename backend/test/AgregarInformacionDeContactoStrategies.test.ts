...
const correo = new AgregarCorreoStrategy(validadoraCorreo);
const correoReceptor = new AgregarCorreoReceptorStrategy(validadoraCorreo);
const redSocial = new AgregarRedSocialStrategy(new ValidadoraRedSocial());
...
test('AgregarCorreoStrategy valida el Value Object y rechaza inválidos/duplicados', () => {
  ...
  // El correo público usa la misma ValidadoraCorreo y la misma traducción por reason.
  for (const [invalido, mensaje] of [
    ['correo@gmail.c', /extensión válida/],
    ['correo@gmail', /dominio completo/],
  ] as const) {
    assert.throws(
...
test('AgregarCorreoReceptorStrategy cambia el Aggregate y devuelve solo el valor canónico', () => {
  const info = informacion();
  const resultado = correoReceptor.ejecutar(info, {
  ...
  assert.equal('displayOrder' in resultado, false);
  assert.equal('companyProfileId' in resultado, false);
});

test('correo público y receptor comparten la traducción de ValidadoraCorreo', () => {
  for (const strategy of [correo, correoReceptor]) {
    assert.throws(
      () => strategy.ejecutar(informacion(), { tipo: strategy === correo ? TIPO_CORREO : TIPO_CORREO_RECEPTOR, datos: { correo: 'usuario@gmail.c' } }),
      /El dominio del correo electrónico debe tener una extensión válida/,
    );
  }
});
