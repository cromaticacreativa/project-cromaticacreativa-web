 ...
 * - Agregar (HU22/HU24): `phone`/`email`/`social_link`/`location` create.
 * - Modificar (HU23/HU25): `phone`/`email`/`social_link`/`location` update (por id
 *   en `meta.keys`) hacia los endpoints `/modify`.
 * - Correo receptor: `company_profile` update, solo cuando cambia ese campo.
 * La eliminación NO pasa por el Hook (es administrada directamente por Directus,
 * sin regla de negocio). Las rutas y operaciones son explícitas, no un validador
 * genérico.
 *
...
const CONTACT_INFORMATION_MODIFY_PATH = '/internal/cms/company-profile/contact-information/modify';
const LOCATION_COLLECTION = 'location';
const LOCATION_PATH = '/internal/cms/company-profile/location';
const LOCATION_MODIFY_PATH = '/internal/cms/company-profile/location/modify';
...
export default ({ filter }, { logger, env }) => {
  for (const coleccion of CONTACT_INFORMATION_COLLECTIONS) {
    ...
    filter(`${coleccion}.items.update`, async (payload, meta) => modificarChild(CONTACT_INFORMATION_MODIFY_PATH, coleccion, payload, meta, { logger, env }));
  }
  ...
  filter(`${LOCATION_COLLECTION}.items.update`, async (payload, meta) => modificarChild(LOCATION_MODIFY_PATH, LOCATION_COLLECTION, payload, meta, { logger, env }));
...
/**
 * Modificación (HU23/HU25): valida el update de un child en NestJS antes de que
 * Directus persista. El id del registro viene en `meta.keys` (Directus 12.3.0).
 * El backend valida contra el estado actual (excluyendo el propio registro en los
 * duplicados) y devuelve el/los valor(es) canónico(s); Directus hace el UPDATE
 * final. Fail closed: si el backend rechaza, se lanza y NO se persiste. Solo se
 * admite un id por operación (el flujo administrativo edita de a uno).
 */
async function modificarChild(ruta, coleccion, payload, meta, ctx) {
  const id = idUnicoDeMeta(meta);
  if (id === null) {
    ctx.logger?.warn?.(`CompanyProfile: update de '${coleccion}' sin un id único; operación cancelada.`);
    throw errorTecnico();
  }
  const canonico = await pedirBackend(ruta, coleccion, { collection: coleccion, id, payload }, ctx);
  // Allowlist estricta (igual que CREATE): Directus aplica SOLO los campos
  // aprobados por el backend. Los campos que el navegador haya enviado
  // (company_profile_id, display_order, id, u otros) NO sobreviven; el id del
  // registro viaja en meta.keys y no necesita volver en el payload.
  return canonico;
}

function idUnicoDeMeta(meta) {
  const keys = meta && Array.isArray(meta.keys) ? meta.keys : [];
  if (keys.length !== 1) return null;
  const k = keys[0];
  return typeof k === 'string' || typeof k === 'number' ? String(k) : null;
}
