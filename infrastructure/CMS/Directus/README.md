...
`extensions/company-profile/` es la extensión estable de tipo `hook` (Directus 12.3.0) del módulo CompanyProfile. Se organiza por módulo/Bounded Context, no por Historia de Usuario, y crece de forma incremental. Su alcance **implementado actualmente** cubre **solo la creación** de dos capacidades, con rutas explícitas:
...
No interviene en `update`, `delete` ni en otras colecciones; HU23 (modificar contacto), HU25 (modificar ubicación) y la eliminación se implementarán en esta misma extensión, sin crear extensions `hu23-*`/`hu25-*`.
...
Variables (en `.env`, nunca versionadas): `BACKEND_INTERNAL_URL`, `BACKEND_INTERNAL_TOKEN` (idéntico a `CMS_INTERNAL_TOKEN` del backend) y, opcional, `BACKEND_INTERNAL_TIMEOUT_MS` (por defecto 5000). Si faltan URL o token, el Hook cancela toda creación sin contactar al backend.

La extensión es JavaScript ESM sin build: Directus la carga desde su `package.json` (`directus:extension.type = hook`) y la habilita automáticamente. Directus sigue siendo el único escritor final; TypeORM Migrations conserva la autoridad estructural.

## Información General — UI administrativa (HU22 y HU24)

Experiencia administrativa amigable para que el Administrador **liste y agregue** los datos públicos de la empresa. Cubre la fase de **AGREGAR** de HU22 (teléfonos, correos, redes sociales) y HU24 (ubicación). **HU23** (modificar contacto) y **HU25** (modificar ubicación) y la eliminación **no** están implementadas y esta UI no las habilita.

### Mecanismo elegido: configuración nativa de Data Model (no un Module compilado)

Se usa **metadata nativa de Directus** (colecciones y campos), no una App Extension (Module) compilada. Razones:

- El toolchain de extensiones de app (`@directus/extensions-sdk` + Vue + Vite) **no está instalado** y agregarlo sería una dependencia pesada, contraria a "no cambiar paquetes por sospechas". La configuración nativa no requiere build.
- La metadata nativa consigue la experiencia pedida (secciones "Teléfonos", "Correos públicos", "Redes sociales", "Ubicación" con listar + agregar, campos técnicos ocultos, textos en español) **y** conserva el flujo de escritor único: cada alta pasa por la **Directus Items API** y dispara el Filter Hook existente.
...
- Campos de negocio con etiquetas/ayudas/placeholders en español y `required` (asterisco de obligatorio, no de error):
  ...
  - **Dirección** — ayuda con ejemplo de dirección completa; sigue siendo **manual** (sin geocoding).
...
Para respetar el alcance (sin editar/eliminar mientras HU23/HU25 no existan) se recomienda un rol **no-administrador** con lectura y actualización limitada del singleton a sus alias relacionales, `create` + `read` sobre `phone`/`email`/`social_link`/`location` y **sin** `update`/`delete` en los children. El rol Administrador **omite** permisos (Directus), por lo que la operación de negocio debe hacerse con ese rol editor. No se automatiza aquí por la API de policies/permissions de Directus 11+; configúrelo en Settings → Access Control.
...
- **12 casos vía Items API → Hook → NestJS → Domain → Directus → MySQL** (todas las mutaciones pasaron por el Hook; NestJS validó sin persistir; los mensajes de negocio llegaron a la respuesta; los inválidos/duplicados no se persistieron):
...
- **Leaflet** con tiles de **OpenStreetMap** (`https://{s}.tile.openstreetmap.org/...`). Se conserva la **atribución obligatoria** "© OpenStreetMap contributors" (no se oculta). Revise la [Tile Usage Policy](https://operations.osmfoundation.org/policies/tiles/): apta para tráfico bajo como este panel; el proveedor de tiles puede cambiarse en el futuro editando la URL de `L.tileLayer` en la extensión, sin tocar Domain/DB.
...
**Diagnóstico** (leído del código de Directus 12.3.0, `@directus/api/dist/app.js`): la CSP por defecto de Directus define `img-src` = `'self' data: blob: https://raw.githubusercontent.com https://avatars.githubusercontent.com` — **sin el host de tiles de OpenStreetMap**. Por eso el contenedor, los controles (+/−), el marcador y la atribución se ven (vienen del CSS/JS de Leaflet) pero **las tiles no cargan** (sus `<img>` son bloqueadas por CSP). En cambio `connect-src` por defecto es `'self' https://* wss://*`, así que **Nominatim (la búsqueda) ya funciona** sin tocar nada.

**Solución**: habilitar el host de tiles en `img-src`. Directus lee la variable con el cast **`array:`** (valores separados por coma) y **sustituye** la directiva completa, por lo que se incluyen los valores por defecto **más** el host de tiles. `connect-src` **no** se modifica (evita reemplazar el `https://*` por defecto). En el `.env` de Directus (ver `.env.example`):
...
La CSP se lee **al iniciar**: (1) detener Directus, (2) actualizar `.env`, (3) reiniciar. Un refresh del navegador no basta. La URL de tiles usada es la oficial de un solo host `https://tile.openstreetmap.org/{z}/{x}/{y}.png` (sin subdominios `{s}`), lo que mantiene la CSP a un solo host y conserva la atribución "© OpenStreetMap contributors". Si tras esto ninguna tile carga (fallo real, no una tile suelta), la UI muestra "No fue posible cargar el mapa. Inténtalo nuevamente.".
...
Cada registro de las cuatro secciones ofrece una acción **Eliminar** (papelera). Como **eliminar no tiene regla de negocio adicional**, la operación va **directa a Directus** (no pasa por NestJS ni por un Hook): `DELETE /items/<phone|email|social_link|location>/:id` con la **sesión autenticada** (`useApi`; sin `BACKEND_INTERNAL_TOKEN`/`DIRECTUS_CONFIG_TOKEN`). Se hace **por id del child** (para `location`, su PK `company_profile_id`), nunca anulando `company_profile_id` ni con un update anidado.
...
- **CREATE vs DELETE**: crear requiere backend (Hook → NestJS); eliminar es Directus directo. Con el backend **apagado**: un create falla (correcto), pero una eliminación **funciona** (es administrada por Directus). Si en el futuro aparece una regla de negocio al eliminar, deberá introducirse Application/Domain + un Hook de delete y su endpoint.

### Correo receptor de solicitudes (guardado propio, sin Guardar global)

El correo receptor **se edita y guarda dentro del gestor**, con su input y su propio botón **"Guardar"** (inline en desktop, apilado en móvil; el campo nativo del singleton queda `hidden`). El botón está **deshabilitado si el valor no cambió** respecto al persistido, muestra "Guardando…" durante el envío y "Correo actualizado correctamente." al aprobar. Al pulsarlo, el gestor hace `PATCH /items/company_profile` con la sesión (`useApi`; sin tokens) → Filter Hook `company_profile.items.update` (**solo si el payload incluye `contact_request_recipient_email`**) → `POST /internal/cms/company-profile/contact-request-recipient-email` → NestJS valida + canonicaliza con el Value Object `EmailAddress` (**no** guarda) → Directus persiste el canónico o aborta. Persistencia inmediata; **no** depende del Guardar global. Backend caído → genérico y **no** guarda. Otros campos del singleton no se interceptan; HU23 completa sigue pendiente.

Mensajes inline (específicos del correo receptor): obligatorio → "El correo receptor de solicitudes es obligatorio."; vacío → "…no puede estar vacío."; formato → "…debe tener un formato válido, por ejemplo: contacto@empresa.com."; sin extensión (`contacto@gmail`) → "…debe incluir un dominio completo, por ejemplo: contacto@empresa.com."; extensión incompleta (`contacto@gmail.c`) → "El dominio del correo receptor de solicitudes debe tener una extensión válida, por ejemplo: .com, .net u .org."; longitud → "…supera la longitud máxima permitida.".
...
3. **Correos**: `aaaa` → "La dirección de correo electrónico no es válida."; válido → persiste.
4. **Redes**: red vacía + URL `hola` → dos errores juntos; válido → persiste; "Otra…" + nombre → guarda el nombre (no "Otra").
5. **Ubicación**: con la CSP de tiles aplicada y Directus reiniciado, "Agregar" en Ubicación → el mapa muestra **calles reales** (no gris); zoom +/− funciona; buscar "Plaza Altamira Caracas" → resultados → elegir centra el mapa y mueve el marcador; click en el mapa y arrastrar el marcador cambian lat/lon; la Dirección **no** cambia sola. Guardar (una vez) → persiste; coordenadas manipuladas fuera de rango vía API → backend rechaza; segunda ubicación → rechazada.
...
8. **Backend apagado**: "Crear nuevo" (cualquier sección) → "No fue posible procesar la solicitud en este momento.", sin INSERT. Pero **Eliminar** con el backend apagado **sí funciona** (es Directus directo) — demuestra la separación CREATE (requiere backend) vs DELETE (Directus directo).
