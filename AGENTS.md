...
- **Patrón CRUD administrativo global (OBLIGATORIO para todo módulo CMS futuro):** **GET** y **DELETE** los resuelve Strapi directo a MySQL; **CREATE/UPDATE con negocio** van Strapi → NestJS → payload canónico → Strapi → MySQL. NestJS no es el escritor final; **fail closed** si NestJS falla. NO crear QueryHandlers/DeleteHandlers en NestJS solo por el CMS. Detalle en `docs/CONVENTIONS.md` ("Patrón CRUD administrativo"). Implementado para CompanyProfile (ADR-028); Portfolio/Services lo seguirán igual.
- **RBAC de servidor:** cada ruta admin exige `admin::isAuthenticatedAdmin` + `admin::hasPermissions` con `admin::company-profile.{read,create,update,delete}`. La UI (useRBAC) solo oculta/inhabilita acciones; la barrera real es el servidor. `CMS_INTERNAL_TOKEN`/`BACKEND_INTERNAL_URL` son solo de servidor (nunca en el bundle Admin).
- Para **CompanyProfile**, la frontera interna `/internal/cms/company-profile/*` (`CompanyProfileCmsController`) está **registrada y protegida** por `CmsServiceAuthGuard` (token técnico `Bearer CMS_INTERNAL_TOKEN`, service-to-service, fail closed; no autentica personas) e incluye `initialize` para crear el singleton. Solo cubre CREATE/UPDATE (ADR-028). No introducir login administrativo en NestJS. El Filter Hook/extension de Directus permanecen eliminados; no recrearlos. Portfolio y Services aún no tienen integración CMS.
- La integración CMS de CompanyProfile está implementada: server-side en `infrastructure/CMS/Strapi/src/company-profile/server/` (repositorio Knex sin content-types, cliente NestJS fail-closed, servicio), rutas admin registradas en `src/index.ts` (protegidas por `admin::isAuthenticatedAdmin`) y Admin UI en `src/admin/` (Información General, OSM, branding). No modelar tablas de negocio como content-types. Pendiente: marcador OSM arrastrable, E2E con admin autenticado, y Portfolio/Services.
